...
async function openMeetingPage() {
  ...
  try {
    ...
    $("assignedUser").innerHTML = selectedMeeting.participants.map(u => `<option value="${u.id}">${escapeHtml(u.name)}${u.email ? " — " + escapeHtml(u.email) : ""}</option>`).join("");
...
function renderDetectedTasks(tasks) {
  ...
  const options = selectedMeeting.participants.map(u => `<option value="${u.id}">${escapeHtml(u.name)}${u.email ? " — " + escapeHtml(u.email) : ""}</option>`).join("");
...
function taskProgress(status) {
  const map = { Pending: 25, Submitted: 75, Completed: 100, Rejected: 25 };
  return map[status] ?? 25;
}
function progressHtml(status) {
  const pct = taskProgress(status);
  const label = status === "Completed" ? "Completed" : status === "Submitted" ? "Work submitted — awaiting approval" : "Assigned — work in progress";
  return `<div class="task-progress"><div class="progress-head"><span>Work progress</span><strong>${pct}%</strong></div><div class="progress-track"><div class="progress-fill" style="width:${pct}%"></div></div><div class="progress-steps"><span class="${pct >= 25 ? "done" : ""}">Assigned</span><span class="${pct >= 75 ? "done" : ""}">Submitted</span><span class="${pct >= 100 ? "done" : ""}">Approved</span></div><p class="meta progress-label">${label}</p></div>`;
}
function taskHtml(t, mine) {
  const assignedTo = t.assigned_to?.name || t.assigned_to_name || "Unassigned";
  const assignedBy = t.assigned_by?.name || t.assigned_by_name || "Unknown";
  const person = mine ? `Assigned by: <strong>${escapeHtml(assignedBy)}</strong>` : `Assigned to: <strong>${escapeHtml(assignedTo)}</strong>`;
